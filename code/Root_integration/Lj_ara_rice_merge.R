...
# Ara
Ara_root <-  readRDS(file = "C:/BGI/root_integration/data/Ara/combined_Ara_annotated_doubletremoved.rds")
...
species.liger <- selectGenes(species.liger, var.thres= 0.1,unshared = TRUE, 
                             unshared.datasets = list(1,2,3), unshared.thresh= c(0.4,0.2,0.4),
                             do.plot = T)
## for ara and lot only
species.liger <- selectGenes(species.liger, var.thres= 0.1,unshared = TRUE, 
                             unshared.datasets = list(2), unshared.thresh= 0.3,
                             do.plot = T)
...
umap_plots <- plotByDatasetAndCluster(species.liger, 
                                      ...
                                      return.plots = TRUE) 
pdf(file = 'C:/BGI/root_integration/figures/Ara_lot_rice_integrated_atlas.pdf',
    width = 7, height = 5.5)
umap_plots[[1]] 
dev.off()
...
##cluster
pdf(file = 'C:/BGI/root_integration/figures/Ara_lot_rice_integrated_atlas_cluster.pdf',
    width = 7, height = 5.5)
umap_plots[[2]] 
dev.off()

###using original cell annotation
Ara_anno <- Ara_root$celltype     
lot_anno <- Lj_root$celltype
rice_anno <- rice_root$celltype

names(Ara_anno) <- paste("Ara", names(Ara_anno), sep = "_")
names(lot_anno) <- paste("LOT", names(lot_anno), sep = "_")
names(rice_anno) <- paste("Rice", names(rice_anno), sep = "_")
## ara
